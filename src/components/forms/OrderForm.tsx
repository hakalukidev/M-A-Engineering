...
function PaymentMethodBadge({ id }: { id: string }) {
  ...
  return (
    <span className="relative block h-11 w-full overflow-hidden rounded-xl bg-white">
      <Image
        src={logo.src}
        alt={logo.alt}
        fill
...
export function OrderForm({
  ...
  if (status === "success") {
    return (
      <div className="flex flex-col items-center gap-3 rounded-xl border border-brand-green/20 bg-brand-green/5 p-10 text-center">
    ...
    <div className="grid grid-cols-[minmax(0,1fr)] gap-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-start lg:gap-10">
        ...
        <div className="rounded-xl border border-brand-ink/10 bg-brand-card p-5 shadow-sm sm:p-6">
              ...
              {selectedProduct.sizes.map((option) => {
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "rounded-xl border-2 px-3 py-1.5 text-xs font-semibold transition-colors",
                      isSelected
          ...
          {selectedProduct && (
            <div className="mt-3 flex items-center gap-3 rounded-xl border border-brand-green/15 bg-brand-cream/60 p-3">
              <span className="relative h-14 w-14 shrink-0 overflow-hidden rounded-xl bg-gradient-to-b from-brand-cream to-brand-ink/10">
                <Image
      ...
      <aside className="order-1 min-w-0 lg:order-2 lg:sticky lg:top-24">
        <div className="space-y-4 rounded-xl border border-brand-ink/10 bg-brand-card p-5 shadow-sm">
          ...
          <div className="flex items-start gap-2 rounded-xl bg-brand-cream/60 p-3 text-xs text-brand-muted">
